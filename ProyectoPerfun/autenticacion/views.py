...
class HelloFromCookieView(APIView):
    def get(self, request):
        ...
        if not access_token:
            return Response({"detail": "No se encontró el token"}, status=401)

        jwt_authenticator = JWTAuthentication()
        try:
            validated_user, _ = jwt_authenticator.authenticate(request._request)
        except Exception as e:
            return Response({"detail": f"Token inválido: {str(e)}"}, status=401)

        return Response({"message": f"Hola, {validated_user.username}, estás autenticado"})
